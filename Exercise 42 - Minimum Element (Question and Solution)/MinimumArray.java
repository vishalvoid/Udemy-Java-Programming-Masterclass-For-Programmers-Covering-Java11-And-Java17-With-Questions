...
public class MinimumArray {
    ...
    MinimumArray min = new MinimumArray();
        System.out.println("Please Enter a value;");
        int number = scan.nextInt();
        scan.nextLine();
        System.out.println("Please Enter " + number + " numbers");
       int[] returnedArray = min.readInteger(number);
        System.out.println("reaturned Array is " + Arrays.toString(returnedArray));
       min.FindMin(returnedArray);

    }
    private int counter;

    private int[] readInteger(int count){
        this.counter = count;
    ...
    private void FindMin(int[] ArrayAsParimeter){
        int[] array = ArrayAsParimeter;
        Arrays.sort(array);
